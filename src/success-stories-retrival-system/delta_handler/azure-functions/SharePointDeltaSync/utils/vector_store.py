"""
Vector Store v1 - Production Ready
PostgreSQL with pgvector for storing and retrieving embeddings.
Implements chunk-level storage with story aggregation.
Includes connection pooling, retry logic, and robust error handling.
"""
    ...
    def _get_connection(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.warning(f"⚠️ Connection attempt {attempt + 1}/{max_retries} failed: {e}")
    ...
    def insert_story(self, story_id: str, category: str, source_file: str,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    self.logger.warning(f"⚠️  Retry {attempt + 1}/{max_retries} for story '{story_id}': {e}")
                    time.sleep(2)
                    continue
                else:
